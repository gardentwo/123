...
$checkQuery = $db->prepare("SELECT id FROM test3 WHERE user_id = ?");
...
if ($result->num_rows > 0) {
    ...
    $stmt = $db->prepare("UPDATE test3 SET answer_right = ?, answer_wrong = ? WHERE user_id = ?");
    $stmt->bind_param("iii", $correctAnswers, $wrongAnswers, $userId);
} else {
    // Создаем новую запись
    $stmt = $db->prepare("INSERT INTO test3 (user_id, answer_right, answer_wrong) VALUES (?, ?, ?)");
